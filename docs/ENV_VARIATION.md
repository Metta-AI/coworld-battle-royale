...
| `dropWeaponOnDeath` | bool / `false` | `true` enables the FFA drop rule | Off by default; armed in the shipped BR configs as of GV46. When enabled, a non-unarmed victim leaves a one-use gun at the death site, consumed by the first living player of a strictly lower tier in pickup range. Measure from `Death.amount`, dropped pickup tokens, existing event tick/x/y data, and existing position tracking for nearby non-collectors. |

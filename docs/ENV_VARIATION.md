...
| `ffaLootRadius` | int / `180` (`FfaLootRadius`) | `ffaLootRadius` | `>=1` (ffa only) | Maximum center-cluster radius in map pixels; center items are bounded by the arena, while low/mid/heavy gun families use deterministic arena-anchored outer, intermediate, and center bands. |

...
- A **paint puddle is a 64×64 px floor hazard** that draws as a violet
  paint spill on the ground. It never blocks movement, bullets, or vision,
  and never slows anything — its only rule is damage over time.
...
- **Puddles are config-gated and ship in NO map by default.** `mapPuddles`
  ...
  count anchors its extra puddle dead center. Spots are drawn from the map
  seed on open floor clear of walls, trenches, other puddles, and both
  teams' base pockets; the exact set is pinned in the replay's `mapSpec`, so
  playback never re-rolls it. 4-team maps do not support puddles yet (an
...
| Puddle size (`PuddleSize`) | 64px | Side of a paint-puddle hazard square |
...
tail on spaces, each corner on the comma). A puddle is a pure floor hazard:

...
const
  ...
  FireSlackPx = 11.0          # CTF's perpendicular-miss margin
  FireMaxToleranceBrads = AimBrads div 8
                              # cap the close-range body-width tolerance at
                              # 32 brads; long-range tolerance bottoms out at
  ...
  FfaHunterMeyerCloneDefault = true
  FfaHunterMeyerArmRadiusDefault = 520.0
  FfaHunterMeyerTangentLeadDefault = 0.5
...
type
  ...
  Actor = object              # a player visible this frame
    ...
    weaponTier: int           # visible FFA held-weapon rig; 0 = unarmed/unknown
  ...
  Bot = ref object
    ...
    ffaAimTracks: seq[Track]  # visible FFA actors with CTF-style velocity leads
...
var
  ...
  FfaHunterMeyerClone = FfaHunterMeyerCloneDefault
  FfaHunterMeyerArmRadius = FfaHunterMeyerArmRadiusDefault
  FfaHunterMeyerTangentLead = FfaHunterMeyerTangentLeadDefault
...
proc ffaHunterMeyerSearchTarget(bot: Bot, me, center: Vec,
    ringRadius: int): Vec =
  ## Returns the moving band target reconstructed from meyer-svg replays.
  let
    fromCenter = me - center
    bearing =
      if fromCenter.len() >= FfaBearingEpsilon: norm(fromCenter)
    ...
    tangent =
      if floorMod(bot.slot, 2) == 0:
        vec(-bearing.y, bearing.x)
      else:
        vec(bearing.y, -bearing.x)
    searchBearing = norm(bearing + tangent * FfaHunterMeyerTangentLead)
    radius = float(max(1, ringRadius)) * FfaPassiveBand
  center + searchBearing * radius
...
proc fireToleranceBrads(distance: float): int =
  ## Convert the sim's bullet corridor plus solid body footprint into the
  ## target's angular half-width at this range. Keep the long-range floor at
  ## the normal combat deadband and cap only the close-range extreme.
  let
    d = max(1.0, distance)
    halfWidth = float(SimTypes.PlayerHalf) + SimTypes.BulletHalfWidth
    radians = arctan(halfWidth / d)
    brads = int(round(radians * float(AimBrads div 2) / PI))
  clamp(brads, CombatDeadband, FireMaxToleranceBrads)

proc aimRotateBits(desiredAim, currentAim, deadband: int): uint8 =
  ## Shared shortest-arc turret controller used by CTF and FFA.
  let err = bradsErr(desiredAim, currentAim)
  if err > deadband: ButtonB
  elif err < -deadband: ButtonSelect
  else: 0'u8
...
proc ffaActorsFor(client: ProtocolClient): seq[Actor] {.measure.} =
  ...
  for (objectId, x, y, width, height, label) in client.spriteObjects():
    let tier =
      if label.startsWith(LabelPrefixCogHeavyGun): FfaWeaponHeavy
      elif label.startsWith(LabelPrefixCogMidGun): FfaWeaponMid
      elif label.startsWith(LabelPrefixCogLowGun): FfaWeaponLow
      elif label.startsWith(LabelPrefixCogGun): FfaWeaponMid
      elif label.startsWith(LabelPrefixCogSprayCan): FfaWeaponHeavy
      else: -1
    if tier < 0:
      continue
    let o = (objectId: objectId, x: x, y: y, width: width, height: height,
      label: label)
    ...
    for i in 0 ..< result.len:
      ...
      if d < bestD:
        ...
        best = i
    if best >= 0:
      result[best].weaponTier = tier
  for (o, label) in client.spriteObjectsWithLabelPrefix(LabelPrefixIdentity):
    let tier =
      if label.endsWith(" spray"): FfaWeaponHeavy
      elif label.endsWith(" heavy gun"): FfaWeaponHeavy
      elif label.endsWith(" mid gun"): FfaWeaponMid
      elif label.endsWith(" low gun"): FfaWeaponLow
      elif label.endsWith(" fist"): 0
      else: -1
    if tier < 0:
      continue
    let p = client.mapPos(o)
...
proc bestFfaGun(client: ProtocolClient, me, center: Vec,
  ...
  var bestDist = 1e18
  let safeLimit = max(0.0, float(max(1, safeRadius)) - safeMargin)
...
proc ffaWeaponFireRange(weaponTier: int): float =
  if weaponTier <= 0:
    70.0
  elif weaponTier == FfaWeaponLow:
    float(FfaLowGunRange)
  else:
    520.0

proc ffaHunterGunStillValid(bot: Bot, client: ProtocolClient,
    actors: seq[Actor], me, center: Vec, ringRadius: int,
    maxDistance = FfaHunterArmTripMaxDetourRadiusDefault): bool =
  ## Re-checks a committed Hunter gun trip against current safety limits.
...
proc hunterFfaIntent(bot: Bot, client: ProtocolClient, actors: seq[Actor],
  ...
  if not FfaHunterArm:
    ...
    bot.ffaLootStartedTick = 0
    return
  let meyerClone = FfaHunterMeyerClone and FfaDoctrine == FfaHunter
  let detourRadius =
    if meyerClone:
      FfaHunterMeyerArmRadius
    else:
  ...
  if ffaHunterGunStillValid(
    ...
    result = ffaBandIntent(bot, me, center, ringRadius, FfaPassiveBand,
      if meyerClone: "MEYER_LOOT" else: "LOOT",
      if meyerClone: "meyer_loot_trip" else: "loot_trip",
      if meyerClone: "move_meyer_gun" else: "move_gun")
  ...
  if gun.found:
    ...
    let opening = ffaElapsedGameTicks(bot) < 30 * TargetFps
    result = ffaBandIntent(bot, me, center, ringRadius, FfaPassiveBand,
      if opening: "MEYER_OPEN_SEARCH" else: "MEYER_LATE_SEARCH",
      if opening: "meyer_open_search" else: "meyer_late_search",
      "move_meyer_search")
    result.moveTarget = bot.ffaHunterMeyerSearchTarget(
      me,
      center,
      ringRadius
    )
...
proc ffaRingUnstickBits(bot: Bot, me, center: Vec): uint8 =
  ...
  if (bot.tick div TargetFps + bot.slot) mod 2 != 0:
    side = side * -1.0
  let candidates = [
    side,
    side * -1.0,
    norm(side + inward * 0.5),
    norm(side * -1.0 + inward * 0.5),
    inward
  ]
...
proc decideFfa(bot: Bot, client: ProtocolClient): uint8 {.measure.} =
  ...
  var hunterSupported = false
  if targetIndex >= 0:
    for i, actor in actors:
      if i != targetIndex and
  ...
  let
    ...
    hunterRingSafety = FfaDoctrine == FfaHunter and
      intent.action == "retreat_ring"
    meyerUnarmedScan = FfaDoctrine == FfaHunter and
      FfaHunterMeyerClone and unarmed and targetIndex < 0
  var action = intent.action
  if meyerUnarmedScan:
    action = "meyer_unarmed_scan"
  ...
  if targetIndex >= 0:
    ...
    var bestTrackD = TrackMatchDist
    for track in bot.ffaAimTracks:
  ...
  if getEnv("CTF_BOT_TRACE").len > 0 and traceInMatch:
    ...
    if targetIndex >= 0:
      ...
      elif abs(bradsErr(desiredAim, bot.estAim)) >
          fireToleranceBrads(targetDist):
  ...
  artFrame(FrameSnap(tick: bot.tick, alive: true,
    ...
    lootTripStarted: intent.lootTripStarted))
  mask
...
proc decide(bot: Bot, client: ProtocolClient): uint8 {.measure.} =
  ...
  let rotBits =
    if desiredAim >= 0:
      aimRotateBits(desiredAim, bot.estAim, deadband)
    else:
      0'u8
...
proc runBot(url: string) =
  ...
  FfaHunterArm = parseEnvBool("CTF_BOT_FFA_HUNTER_ARM",
    FfaHunterArmDefault)
  ...
  FfaHunterMeyerClone = parseEnvBool(
    "CTF_BOT_FFA_HUNTER_MEYER_CLONE",
    FfaHunterMeyerCloneDefault)
  FfaHunterMeyerArmRadius = max(1.0, parseEnvFloat(
    "CTF_BOT_FFA_HUNTER_MEYER_ARM_RADIUS",
    FfaHunterMeyerArmRadiusDefault))
  FfaHunterMeyerTangentLead = max(0.0, parseEnvFloat(
    "CTF_BOT_FFA_HUNTER_MEYER_TANGENT_LEAD",
    FfaHunterMeyerTangentLeadDefault))
  ...
  echo "baseline slot=", slot, " team=", bot.team, " role=", bot.role,
    ...
    " ffaHunterMeyerClone=", FfaHunterMeyerClone,
    " ffaHunterMeyerArmRadius=", FfaHunterMeyerArmRadius,
    " ffaHunterMeyerTangentLead=", FfaHunterMeyerTangentLead,

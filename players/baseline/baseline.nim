...
const
  ...
  FfaHunterUpgradeDetourRadiusDefault = 320.0
...
var
  ...
  FfaHunterUpgradeDetourRadius = FfaHunterUpgradeDetourRadiusDefault
...
proc hunterFfaIntent(bot: Bot, client: ProtocolClient, actors: seq[Actor],
  ...
  if not FfaHunterArm:
    ...
    return
  if not unarmed and
      (FfaDoctrine != FfaHunter or weaponTier >= FfaWeaponHeavy):
    bot.ffaLootTrip = false
    bot.ffaLootTargetValid = false
    bot.ffaLootTargetTier = 0
    bot.ffaLootStartedTick = 0
...
proc runBot(url: string) =
  ...
  FfaHunterUpgradeDetourRadius = max(1.0, parseEnvFloat(
    "CTF_BOT_FFA_HUNTER_UPGRADE_DETOUR_RADIUS",
    FfaHunterUpgradeDetourRadiusDefault))
  ...
  echo "baseline slot=", slot, " team=", bot.team, " role=", bot.role,
    ...
    " ffaHunterUpgradeDetourRadius=", FfaHunterUpgradeDetourRadius,

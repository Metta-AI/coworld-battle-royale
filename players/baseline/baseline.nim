...
proc ffaRingUnstickBits(bot: Bot, me, center: Vec): uint8 =
  ...
  let flipSide =
    if bot.ffaRingUnstickAttempts mod 2 == 0:
      bot.ffaRingUnstickBaseFlip
    else:
      not bot.ffaRingUnstickBaseFlip
...
proc ffaRingUnstickAction(bot: Bot): string =
  ## Returns telemetry for the current alternating unstick side.
  if bot.ffaRingUnstickAttempts > 0 and
      bot.ffaRingUnstickAttempts mod 2 == 0:
    "ring_unstick_flip"
  else:
    "ring_unstick"

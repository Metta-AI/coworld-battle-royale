...
- `meta.json` — slot, team, role, active compile defines, sample cadence,
  the FFA doctrine, and `gameTicksPerFrame` (artifact schema 5).
...
- `ticks.jsonl` — a state row every 12 ticks (~0.5 s): position, hp, aim,
  ...
  tier/elapsed game seconds, movement target, visible enemies, engage
  distance, engage reason, input mask, and status flags
  (carry/shield/plasma/nade/pushOut).
- `summary.json` — event counters plus per-objective and per-action tick
  histograms, armed fraction, and loot-trip count; the first internal
  telemetry error, if any, is recorded here (telemetry disables itself rather
  than ever touching gameplay).

...
const
  ...
  #   capture-seed6:  record_fixture.sh <out> 6
  ...
  # runs to a time-limit draw, so the capture fixture moved to seed 6.
  # Under the current rules seed 6 ends on a capture (Red captures the blue
  # heart, eliminating Blue). The recording must ALSO keep
  ...
  CaptureFixture = FixtureDir / "capture-seed6.bitreplay"
...
suite "broadcast state channel":
  ...
  test "final frame state names the verdict honestly":
    ...
    try:
      ...
      # seed 6: Red captures the blue heart, eliminating Blue).

...
const
  ...
  # Under the current rules seed 6 ends on a capture (Blue captures the red
  # heart, eliminating Red). The recording must ALSO keep
...
suite "broadcast state channel":
  ...
  test "final frame state names the verdict honestly":
    ...
    try:
      ...
      # seed 6: Blue captures the red heart, eliminating Red).
